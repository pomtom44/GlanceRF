...
if ! "$VENV_PYTHON" -c "import fastapi" 2>/dev/null; then
    ...
    if ! "$VENV_PYTHON" -m pip install -r "$PROJECT_DIR/requirements.txt"; then
        ...
        echo "  Check requirements.txt and your network. On some distros you may need development headers (e.g. python3-dev on Debian/Ubuntu)."
        exit 1
    fi
fi
...
Type=Application
Name=GlanceRF
Comment=GlanceRF dashboard (terminal + browser)
